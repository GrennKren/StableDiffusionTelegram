...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == 'UPSCALE4':
        await context.bot.send_photo(update.effective_user.id, output.tobytes(), caption=f'"{prompt}" (Seed: {seed[0]}) (Size: {width * 4}, {height * 4})', reply_markup=get_try_again_markup(), reply_to_message_id=replied_message.message_id)
    else:
        for key, value in enumerate(im): 
