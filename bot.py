...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == "TRYAGAIN":
        if replied_message.photo is not None and len(replied_message.photo) > 0 and replied_message.caption is not None:
            ...
            im, seed = generate_image(prompt, photo=photo, number_images=1, user_id=replied_message.from.id)
        else:
            prompt = replied_message.text
            prompt = prompt if prompt[0] != "/seed" else " ".join(prompt.split(" ")[1:])
            im, seed = generate_image(prompt, number_images=1, user_id=replied_message.from.id)
